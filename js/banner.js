...
let wrap = document.querySelector('.banner .wrap')
let imglist = document.querySelector('.banner  .imglist')
let btnList = document.querySelector('.banner  .btnlist')
let imgs = document.querySelectorAll('.banner  .imglist > a')
let btns = document.querySelectorAll('.banner .btnlist > a')
let leftctrl = document.querySelector('.banner  .leftctrl')
let rightctrl = document.querySelector('.banner .rightctrl')
// 取wrap的宽度作为每次切换图片移动的距离
let tabSize = wrap.clientWidth
// let tabSize = imglist.clientWidth / 5
// 记录当前图片位置的下标
let imgIndex = 0
leftRight()
autoPlay()
